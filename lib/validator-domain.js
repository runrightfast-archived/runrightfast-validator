 ...
 *					},
 *					connections:{
 *						name:'connections',										// connections is an Array of 'Connection's, which is a Type defined in the above ObjectSchema
 *						type:'Array',
 *						constraints:[
 *							method:'includes',
 *							args:[
...
(function() {
	...
	var validateObjectType = function validateArrayType(obj) {
		var type = joi.types[obj.type]();
		lodash.forEach(obj.constraints, function(c) {
	...
	var validateProperty = function validateType(obj) {
		...
		if (obj.constraints) {
			...
			// lodash.forEach(obj.constraints, function(c) {
			// if (c.method === 'objectSchemaType') {
			// assert(obj.type === 'Object', 'objectSchemaType is only supported
			// by type=\'Object\'');
			//
			// var params = {
			// namespace : c.args[0],
			...
			// };
			// validate(params, objectSchemaTypeParamsSchema);
			// } else if (obj.type === 'Array' && (c.method === 'includes' ||
			// c.method === 'excludes')) {
			//
			// } else {
			// type = type[c.method].apply(type, c.args);
	...
	Type.prototype.getSchema = function(getObjectSchemaFxn) {
		...
		lodash.forEach(this.properties, function(prop) {
			schema[prop.name] = createType(joi.types[prop.type](), prop.constraints);
		});
		return schema;
	};
	...
	var ObjectSchema = function ObjectSchema(options) {
		...
		this.id = objectSchemaId(this.namespace, this.version);
