		...
		lodash.forEach(this.properties, function(prop) {
			schema[prop.name] = lodash.foldl(prop.constraints, function(type, constraint) {
				if (constraint.method === 'objectSchemaType') {
					type.validateObjectSchema = function(constraint) {
						this.add('objectSchemaType', function() {
							var objectSchemaType = getObjectSchemaFxn({
								namespace : constraint.args[0],
								version : constraint.args[1],
								type : constraint.args[2]
							});

							return function(value, obj, key, errors, keyPath) {
								try {
									objectSchemaType.validate(value);
									return true;
								} catch (err) {
